...
bool rotating=false;
//articulation
float snake_rot=0.0, lf_rotate=0, f_rotate=0;
float change = 1;

//
//Prototypes
//
void drawScene(float snake_rot, float lf_rotate, float f_rotate);
void drawSnake(float lf_rotate, float f_rotate);
void drawLFbody(float f_rotate);
...
void drawScene(float snake_rot, float lf_rotate, float f_rotate)
{
	...
	glRotatef(snake_rot, 0.0, 1.0, 0.0);
	drawSnake(lf_rotate, f_rotate);
...
void drawSnake(float lf_rotate, float f_rotate)
{
	...
	glPushMatrix();
		glTranslatef(3, 1, 0);
		glRotatef(lf_rotate, 0.0, 1.0, 0.0);
		drawLFbody(f_rotate);
	glPopMatrix();
}
...
void drawLFbody(float f_rotate)
{
	...
	glPushMatrix();
		glTranslatef(3, 0, 0);
		glRotatef(f_rotate, 0.0, 1.0, 0.0);
		drawF();
	glPopMatrix();	
}
...
void displayCallBack()	
{
    ...
    drawScene(snake_rot, lf_rotate, f_rotate);
...
int main(int argc, char** argv)
{
   // Allow cmd line arguments to be passed to the glut
    ...
    glutCreateWindow("Simple articulation - Snake");
