...
int mouseX0, mouseY0;
bool rotating=false;
static float r_angle, l_angle, f_angle;
...
void drawScene(float r_angle, float l_angle, float f_angle)
{
	
	glPushMatrix();
		glRotatef(r_angle, 0.0, 1.0, 0.0);    
		drawR();
	glPopMatrix();
	glTranslatef(4,1,0);
	glPushMatrix();
		glRotatef(l_angle, 0.0, 1.0, 0.0);
		drawL();
	glPopMatrix();
	glTranslatef(5,0,0);
	glPushMatrix();
		glRotatef(f_angle, 0.0, 1.0, 0.0);
		drawF();
	glPopMatrix();
}
...
void displayCallBack()	
{
    ...
    drawScene(r_angle, l_angle, f_angle);
...
int main(int argc, char** argv)
{
   // Allow cmd line arguments to be passed to the glut
	...
	printf("Key \"1\" - Rotate R.\n");
	printf("Key \"2\" - Rotate L.\n");
	printf("Key \"3\" - Rotate F.\n");
